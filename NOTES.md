...
Can one small macOS process own a hands-free Codex prompt transaction without wrapping Codex or changing how the target application renders it?
...
- The final transcript is pasted into the configured target application, then Return is sent after the paste completes.
...
- Immediate focus and direct Ghostty commands were verified before application targeting was generalized.
...
- Voice phrases, the active application target, and target-scoped command aliases load from `~/.config/voice-control/config.toml`. Valid saves reload automatically; invalid saves leave the last good configuration running.
...
- Whether focusing either target, pasting, and pressing Return remains reliable across repeated prompts, target changes, and macOS Spaces.
