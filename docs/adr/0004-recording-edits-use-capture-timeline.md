...
Apple progressive transcription treats the latest live preview as the authoritative prompt. When a new speech burst begins after a distinct period of silence, the daemon checkpoints the preview before that burst. If the burst is recognized as the submit phrase, the checkpoint is submitted, so a misrecognition such as `Sunday` does not leak into the prompt. Apple finalization and file retranscription are never used for prompt submission. If live recognition produces no usable text, submission fails instead of asking Apple to reinterpret the complete recording.

Each visible progressive preview records the latest prompt audio position it covers. Explicit submission preserves that preview when it already covers the required prompt audio, so closing the live input cannot trigger an Apple rewrite. When the preview is behind, submission closes the live audio input and waits up to one second for a newer progressive callback. If Apple still reports incomplete timestamp coverage, Voice Control submits the newest usable progressive text. If the drain produces no text, it keeps the visible preview.
